...
In this intro lab, we will learn how to use the iOLab device to take useful data. Your iOLab is essentially a microcomputer with a number of sensors suited for measuring quantities such as Force, veclocity, acceleration, Magnetic fields, etc. Your iOLab processes information from these sensors and relays it to the dongle attached to your computer. When the software is running, this data can be recorded and displayed in a graph.
...
After calibration, we will explore how the iOLab allows us to take *qualitative* and *quantitative* measurements. Our first project will be exploring the field of permnenant magnets, and getting comfortable with using the magnetometer to aquire data. You will use the magnetometer throughout this course.
...
2. Perform the calibration for the iOLab device’s $x, y, z$ axis separately. The image of the appropriate iOLab orientation is shown in the lower right corner of the Calibration window. Note that the longest side is the $x$ axis, the second longest side is the $y$ axis, and the shortest side is the $z$ axis. 
...
- - the ellipse is centered around (0, 0). Note that, despite appearances, the plot should be circular. The elliptical shape is an artifact of the $x$ and $y$ axes have different scales on your screen. 
- - $\Delta x$ and $\Delta y$ (the width and height) are equal on the ellipse.
5. If these two criteria are met, you may move on to the next step. Otherwise make a note of your issue and return to the calibration procedure and recalibrate your device, being sure that the area is clear of metal and magnetic objects.
...
(b) After flipping the magnet, describe the observed difference and a possible explanation.
...
(b) Devise a quick experiment which could verify this, and sketch a plot of the expected result. 
